...
import { createCandidate, updateCandidate, getDepartments, getJobs } from "../../services/api.js";
...
export default function CandidateForm({ candidate, onSubmit, onCancel }: CandidateFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let result;
      if (candidate?.id) {
        result = await updateCandidate(candidate.id, submitData);
      } else {
        result = await createCandidate(submitData);
      }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={formData.status} onValueChange={(value) => handleInputChange('status', value)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="3">Shortlisted</SelectItem>
                  <SelectItem value="4">Assessment</SelectItem>
                  <SelectItem value="5">Interview</SelectItem>
                  <SelectItem value="6">Hired</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Button type="submit" disabled={loading}>
              {loading ? 'Saving...' : (candidate ? 'Update' : 'Create')} Candidate
            </Button>
          </div>
        </form>
      </CardContent>
